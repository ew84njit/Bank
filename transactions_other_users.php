...
<form method="POST">
	...
	<input name="account_dest" id="dest" placeholder="Search" value="<?php safer_echo($query); ?>"/>
...
if(isset($_POST["save"])){
	...
	//Account Dest
	$stmtDest = $db->prepare("SELECT id, account_number, user_id, account_type, opened_date, balance 
        from Accounts WHERE account_number like :q LIMIT 10");

    $r = $stmtDest->execute([":q" => "%$query%"]);

    if ($r) {
        $results = $stmtDest->fetchAll(PDO::FETCH_ASSOC);
    }
    else {
        flash("There was a problem fetching the results");
    }
